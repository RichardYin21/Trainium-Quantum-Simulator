...
class QuantumCircuitSimulator(nn.Module):
	...
	def forward(self, state, targets, gates):
		...
		for step, target, gate in zip(range(len(targets)), targets, gates):
			...
			permutation = [0] + [i + 1 for i in permutation] # don't permute the real/imag axis
			...
			# Update: the permuting on CPU hack now dominates the computation time by a wide margin
